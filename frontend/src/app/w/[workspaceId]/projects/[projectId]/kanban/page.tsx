...
function sortTasksForBoard(items: TaskRead[]): TaskRead[] {
  return [...items].sort(
...
export default function KanbanPage() {
  ...
  function resolveDropFromOverId(
    overId: string,
    ...
    if (columnKeys.includes(overId)) {
      ...
      // Easy drop: placing on column body defaults to end.
    ...
    if (overId.startsWith("task-drop:")) {
      ...
      const overTask = (previewTasks ?? orderedTasks).find((t) => t.id === taskId);
      if (!overTask) return null;
      const list = (byStatus.get(overTask.status) ?? []).filter(
        (t) => t.id !== activeTaskId,
      );
  ...
  function computeReorderedTasks(
    all: TaskRead[],
    activeId: string,
    targetStatus: string,
    targetIndex: number,
